...
function [AlphaSet, BetaSet, PsiSet] = MotionDepthJoint(LeftVideoName,RightVideoName)
FrameExt(LeftVideoName);
FrameExt(RightVideoName);
%addpath(genpath('/home/sathya/DeMo3D/Codes/Toolbox'));
%addpath(['/MoDi3D/Codes/' numstr(LeftVideoName)]);
%addpath(['/Mo3D/Codes/' numstr(RightVideoName)]);
LeftFramePath  = ['/MoDi3D/Codes/' numstr(LeftVideoName)];
RightFramePath  = ['/MoDi3D/Codes/' numstr(RightVideoName)];
for i = 1:245
    ...
    for j2=1:3
        for k2=1:6
            ...
            AlphaSet(m,i) = alpha_ld_l;
            BetaSet(m,i) = beta_ld_l;
            PsiSet(m,i) = PsiScore;
            m=m+1;
